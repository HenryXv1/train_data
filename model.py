...
from src.model_utils import LLAVA_NEXT, QWEN2_VL, PHI3V, LLAVA_OV, QWEN2_5_VL, QWEN3_VL, backbone2model, get_backbone_name, print_master
...
class MMEBModel(nn.Module):
    ...
    def build(cls, model_args: ModelArguments, training_args: TrainingArguments=None, **kwargs):
        ...
        if model_backbone == PHI3V:
            ...
            if model_args.rerank_embedding == "embedding":
                if model_backbone == QWEN3_VL and backbone2model[model_backbone] is None:
                    raise ImportError("Qwen3-VL requires a newer transformers; install from source: pip install git+https://github.com/huggingface/transformers")
                config._attn_implementation = "flash_attention_2"
    ...
    def load(cls, model_args: ModelArguments, is_trainable=False, **kwargs):
        ...
        if model_args.model_backbone in {LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL, QWEN3_VL}:
            if model_args.model_backbone == QWEN3_VL and backbone2model[model_args.model_backbone] is None:
                raise ImportError("Qwen3-VL requires a newer transformers; install from source: pip install git+https://github.com/huggingface/transformers")
            config._attn_implementation = "flash_attention_2"
            config.vision_config._attn_implementation = "flash_attention_2"
